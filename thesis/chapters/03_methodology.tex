...
quadratic form $\mathbf{y}^T Q \mathbf{y}$ that reproduces~\eqref{eq:qubo_multi}
has the following block structure. Diagonal entries $Q_{ii}$
combine the modularity contribution $-B_{vv} / (2m)$ with the
linear part of the constraint penalty $-P$. Off-diagonal entries
fall into one of two categories: a \emph{modularity entry}
$Q_{ij} = -B_{vw} / m$ when indices $i$ and $j$ correspond to
different vertices but the same community, and a \emph{constraint
entry} $Q_{ij} = 2P$ when indices $i$ and $j$ correspond to the
same vertex but different communities. All remaining entries are
zero. The resulting matrix has size $nk \times nk$ and is
symmetric.
...
\texttt{SAGEResBlock}, which combines two parallel GraphSAGE
layers with different aggregators followed by a residual addition.
...
original QIGNN architecture supports only bipartite outputs.
...
$\Pb \in \mathbb{R}^{n \times k}$, with no need to materialize $\Q$.
Memory complexity drops from $O(n^2 k^2)$ to $O(n^2 + nk)$, a
factor-of-$k^2$ improvement when the modularity term dominates.
For the Email-EU-core example ($k=42$), this is a reduction from
approximately~$6.9$~GB to approximately~$4$~MB. Without this
reformulation, training on graphs with $n \sim 10^3$ and
$k \sim 10^2$ would not be feasible on commodity hardware. This
memory-efficient implementation is one of the contributions of
this thesis.
...
The multi-class QUBO loss~\eqref{eq:loss_qubo} has a degenerate
global minimum corresponding to the trivial assignment in which
every vertex is placed in the same community $c^*$, that is,
$P_{v,c^*} = 1$ for all $v$ and $P_{v,c} = 0$ for all $c \neq c^*$.
In this state the modularity contribution
\begin{equation*}
...
every vertex. The trivial assignment thus achieves a loss of zero
without recovering any community structure. It is, moreover, a wide
and easily reachable attractor in the loss landscape, requiring no
coordinated agreement between vertices and reached as soon as the
softmax outputs are biased toward any single community. Empirically
the optimization converges to this attractor in a substantial
fraction of training runs (Section~\ref{sec:exp_collapse}).
...
adapted from the supervised DMoN method~\cite{tsitsulin2020graph}
is added to the multi-class loss:
\begin{equation}
...
The algorithm has three notable properties. First, the depth of
the recursion is bounded by $\log_2 |V|$, and in practice it is
much smaller because the modularity-gain criterion terminates
recursion early on graphs whose true number of communities is
moderate. Second, the algorithm produces an interpretable
