...
on bipartite combinatorial problems --- MaxCut and Maximum
Independent Set --- where each variable is a single binary
indicator.

The community detection problem is bipartite only in the special
case of two communities. With $k > 2$ communities each vertex must
...
The multi-class loss has a degenerate global minimum at the trivial
all-in-one assignment, where every vertex is placed into the same
community. An orthogonality regularization adapted from
DMoN~\cite{tsitsulin2020graph} is integrated to discourage this
collapse by penalizing overlap between community columns. Its
...
\begin{enumerate}
    ...
    hierarchical method matches Louvain in modularity within $0.02$
    on every benchmark in the study, including graphs with up to
    forty-two ground-truth communities, while matching or exceeding
    Louvain in normalized mutual information on ten of thirteen
    benchmarks (eight strict wins plus two ties at the perfect
    NMI of $1.0$). The modularity-gain stopping criterion
    automatically discovers the number of communities, recovering
    Louvain's choice of $k$ on graphs where Louvain's heuristic
    is itself reliable.
    ...
    validated only on bipartite problems such as MaxCut and Maximum
    ...
    the supervised DMoN method~\cite{tsitsulin2020graph}; its
