...
The primary baseline is Louvain~\cite{blondel2008louvain};
Leiden~\cite{traag2019leiden} is reported as a sanity-check
secondary baseline (its modularity on the seven main multi-class
graphs differs from Louvain's by at most $0.006$, and it is
therefore not duplicated in every comparison table). Three
...
twenty runs per graph except on Polblogs and Email-EU-core,
where ten runs were used to limit total wall time. The
constraint penalty for the multi-class formulation was set to
$P = 1.5 \cdot \max_{v,w} |B_{vw}|$.
...
at least $80\%$ of vertices into a single community. In the
extended-shots experiment (twenty shots per graph, $k =
k_{\mathrm{true}}$), the unregularized multi-class formulation
collapses on most benchmarks, with rates ranging from $10\%$ on
LFR-$\mu$0.1 to $85\%$ on Karate.
Figure~\ref{fig:collapse_heatmap} reports the same collapse
metric on the earlier regularization-comparison experiment (five
shots per graph, $k$ set to a representative value above
$k_{\mathrm{true}}$); the smaller shot count yields broader
extremes (e.g., $100\%$ on Karate at $k=5$), but the relative
ordering of the strategies is the same.
...
\begin{figure}[H]
    ...
    with orthogonality regularization (five shots per graph). The
    third column shows a collapse-only regularization considered
    during exploratory work; it suppresses collapse but did not
    improve modularity on any benchmark and is not adopted in the
    final method. Orthogonality regularization suppresses
    collapse to zero on five of the seven benchmarks, leaving a
    residual rate of $0.20$ on Football ($k=12$) and $0.60$ on
    Karate ($k=5$); these residual cases are eliminated entirely
    at larger $\alpha$.}
...
\begin{table}[H]
    ...
    regularization ($\alpha = 0.1$, twenty shots, ten on
...
Second, under the fully automatic modularity-gain criterion the
algorithm strictly exceeds Louvain in NMI on four benchmarks
(Dolphins $+0.108$, Polbooks-binary $+0.082$, Polblogs $+0.090$,
SBM-hard $+0.012$) and ties on the two well-separated SBM
variants at perfect NMI of $1.0$. Selecting per graph the best
of the three stopping criteria (with target-$k$ supplied as
input on graphs where it is used) extends the count to
\textbf{ten of thirteen benchmarks} (eight strict wins plus the
two ties), with the largest gains on imbalanced graphs:
$+0.236$ on Polbooks-binary, $+0.094$ on Email-EU-core, and
$+0.083$ on Karate. The remaining three losses are all LFR
benchmarks with low mixing ($\mu \le 0.3$), where Louvain's NMI
is already at or near $1.0$ and the gap is at most $0.058$. The
SBM-hard ``win'' should be read with care: both methods reach
NMI close to random ($0.06$ versus $0.04$), reflecting weak
ground-truth structure at $p_{\mathrm{in}} = 0.2$,
$p_{\mathrm{out}} = 0.15$. Third, the Email-EU-core benchmark is
...
number of communities. On the synthetic benchmarks with
well-separated structure (LFR-$\mu$0.1, LFR-$\mu$0.3, LFR-large,
SBM-easy, SBM-large) and on Polblogs, the criterion recovers
$k_{\mathrm{true}}$ exactly. On the remaining real-world
benchmarks the criterion stops at a value that coincides with
Louvain's own choice rather than with $k_{\mathrm{true}}$
(Karate $k = 4$, Dolphins $k = 5$, Polbooks $k = 5$, Football
$k = 10$, Email-EU-core $k = 8$). This is not a failure of the
criterion but a property of the modularity objective on these
graphs: finer partitions reduce modularity even when finer
ground-truth structure exists. The classical resolution
limit~\cite{fortunato2007resolution} of modularity is the
underlying reason.
