...
inference. The thesis uses Louvain and Leiden as the primary
baselines because they share the modularity objective with the
proposed method, making the comparison meaningful.
...
The contribution of DMoN that is particularly relevant for this
thesis is the orthogonality regularization
$\bigl\| \Pb^T \Pb / \| \Pb^T \Pb \|_F - I_k / \sqrt{k} \bigr\|_F^2$,
which Tsitsulin et al.\ introduce to prevent degenerate cluster
assignments. The same regularization is adopted in
Section~\ref{sec:method_ortho} of this thesis, although in the
...
existing validation of QIGNN is restricted to bipartite
...
guidance by including Louvain and Leiden as primary baselines on a
